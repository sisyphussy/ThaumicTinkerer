...
public class InfusedSeedsPlanter extends PlantableFarmer implements ICompat {
    ...
    protected boolean canPlant(World worldObj, BlockCoord bc, IPlantable plantable) {
        return (super.canPlant(worldObj, bc, plantable)
                && plantable == ThaumicTinkerer.registry.getFirstItemFromClass(ItemInfusedSeeds.class))
                || worldObj.getBlock(bc.x, bc.y - 1, bc.z)
    ...
    public boolean canHarvest(TileFarmStation farm, BlockCoord bc, Block block, int meta) {
        return super.canHarvest(farm, bc, block, meta)
                && block == ThaumicTinkerer.registry.getFirstBlockFromClass(BlockInfusedGrain.class);
    }
    ...
    protected boolean plant(TileFarmStation farm, World worldObj, BlockCoord bc, IPlantable plantable) {
        ...
        if (seeds != null) {
            ...
            ((TileInfusedGrain) farm.getWorldObj().getTileEntity(bc.x, bc.y, bc.z)).primalTendencies = ItemInfusedSeeds
                    .getAspectTendencies(seeds);
        }
        return true;
    }
}
