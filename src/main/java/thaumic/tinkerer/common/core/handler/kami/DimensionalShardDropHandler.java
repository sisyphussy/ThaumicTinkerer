    ...
    public void onEntityLivingDrops(LivingDropsEvent event) {
        if (event.source.getEntity() instanceof EntityPlayer) {
            if (event.entityLiving instanceof EntityEnderman) {
                if (event.entityLiving.dimension == ConfigHandler.endDimensionID && Math.random() <= 1D / 32D) {
                    event.drops.add(
                            new EntityItem(
                ...
                if (event.entityLiving.dimension == ConfigHandler.netherDimensionID && Math.random() <= 1D / 16D) {
                    event.drops.add(
                            new EntityItem(
                                    event.entityLiving.worldObj,
                                    event.entityLiving.posX,
