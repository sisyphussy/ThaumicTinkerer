...
public class KamiResearchItem extends TTResearchItem {
    ...
    public ResearchItem setPages(ResearchPage... par) {
        List<String> requirements = parentsHidden == null || parentsHidden.length == 0 ? new ArrayList()
                : new ArrayList(Arrays.asList(parentsHidden));

        if (!isAutoUnlock()) for (String categoryStr : ResearchCategories.researchCategories.keySet()) {
            ...
            for (String tag : category.research.keySet()) {
                ...
                if (research.isLost() || research.isVirtual()
                        || research instanceof KamiResearchItem
                        || requirements.contains(tag))
                    continue;
                if (research.category.equals(LibResearch.CATEGORY_THAUMICTINKERER) || research.category.equals("BASICS")
                    ...
                    boolean found = false;
                    for (String black : Blacklist) if (tag.startsWith(black)) {
                        found = true;
                    }
