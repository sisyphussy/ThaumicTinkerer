        ...
        if (par2 >= xPosition && par2 < xPosition + 16 && par3 >= yPosition && par3 < yPosition + 16) {
            ...
            for (Aspect aspect : data.aspects.getAspectsSorted()) tooltip.add(
                    " \u00a7" + aspect.getChatcolor()
                            ...
                            + '\u00a7'
                            + "r x "
                            + data.aspects.getAmount(aspect)
                            + " "
                            + StatCollector.translateToLocal("ttmisc.baseCost"));
            if (this instanceof GuiButtonFramedEnchantment && !parent.enchanter.working) tooltip.add(
